...
class QuerystringBuilder(object):
    ...
    def _add_values(self, metric, metric_map, metric_prefix):
        ...
        metric_map[metric_prefix + self._STAT_MAX] = round(metric.statistics.max, 8)
        metric_map[metric_prefix + self._STAT_MIN] = round(metric.statistics.min, 8)
        metric_map[metric_prefix + self._STAT_SUM] = round(metric.statistics.sum, 8)
